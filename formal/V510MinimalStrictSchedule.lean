...
We prove that it has all order/topological properties needed for diagonal
summation and is pointwise no larger than the source doubling envelope.  We
then apply it to the canonical least-admissible local scales from v5.9.
...
theorem strictEnvelope_pos (b : ℕ → ℕ) (n : ℕ) :
  ...
  | succ n =>
      exact lt_of_lt_of_le (Nat.zero_lt_succ _) (strictEnvelope_growth b n)
...
theorem strictEnvelope_lower_bound (b : ℕ → ℕ) (n : ℕ) :
  ...
  | succ n ih =>
      exact (Nat.succ_le_succ ih).trans (strictEnvelope_growth b n)
