...
`CutStageEstimates.exists_finite_diagonal_cut_bounds`, with the same coordinate
bounds, raw field bounds, finite-family aggregation, logarithmic absorption,
and `cut_product_bound_of_threshold`.

The only change is the numerical selector: instead of the source factor-two
envelope, use `V510MinimalStrictSchedule.minimalStrictSchedule`.
-/
